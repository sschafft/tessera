...
- **Tap-to-place builder canvas, single-target model.** Tap an empty cell → a phantom arms there with the dock's current shape/colour/rotation; sculpt at the dock and the phantom updates in place. Tap another empty cell to commit + chain; tap a placed piece to enter edit mode (move, mutate, ⌫ Remove or Del to remove with a 5-second undo toast). Optimistic UI keeps every action snappy — placements echo back to all connected clients within ~200ms via Supabase Realtime.
- **Live correctness + scoring.** Running rounds score continuously; pieces light green/red as they land, the guider sees the same mirrored correctness on the goal canvas, and the GM-tunable wrong-piece penalty can still drive scores negative. There is no separate "Test solution" CTA in v1.3.
...
- **Always-visible briefs + intro overlay.** v1.3 keeps the brief card open for the whole round; a one-shot intro overlay frames *why* it exists the first time a player lands in the role. The earlier sealed/minimised envelope flow was removed.
...
- **Mirrored correctness for the guider.** The guider's goal canvas lights up the same green halos and ✓ badges as placements land, plus a live "X/Y placed" chip — so the guider can see at a glance whether the directions they're giving are landing.
- **End-of-round reflection survey.** Builders and guiders answer a two-question reflection card at round end (who carried the conversation, what made it harder); the game-end view aggregates those responses anonymously for the debrief.
- **Reset pairs.** Pre-round, the GM can use `↺ reset pairs` to wipe allocations and re-seat the room without rebuilding the game. A styled confirm modal protects the action mid-cast.
- **Partner presence + multi-phase timer.** A small green dot beside the partner avatar lights up when they're connected to the game's presence channel. The round timer chip escalates through `normal → warning ≤2m → urgent ≤30s → critical ≤10s` so the wrap-up rhythm is felt across both player and GM surfaces.
...
Tessera runs on the **Vercel + Supabase free tier** and is built around those constraints — see TDD §13 for the per-service guardrails (AI per-game + per-day caps, Realtime drag-broadcast throttle, concurrent-game cap, Vercel `maxDuration`). Library-only games scale to ~50 participants; AI-brief games are capped at 15 because the round-start path preflights briefs sequentially with a ~6 s per-call budget and we want worst-case start to fit the route's 30 s ceiling.
